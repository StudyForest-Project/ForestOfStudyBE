...
// 포인트 계산 함수
function calculatePoints({ targetTime, activeMinutes, pauseUsed }) {
  // 최소 시간 미달 : 0 point
  ...
  if (activeMinutes < targetTime) {
    ...
    return achievementRate >= ACHIEVEMENT_THRESHOLD ? BASE_POINTS : 0;
  }
  ...
  const effectiveMinutes = Math.min(activeMinutes, targetTime * CEILING_MULTIPLIER);
  const bonusPoints = Math.floor(effectiveMinutes / BONUS_UNIT_TIME) * BONUS_POINTS_PER_UNIT;
